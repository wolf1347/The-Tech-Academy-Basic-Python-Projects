...
def center_window(self, w, h): #pass the tkinter frame reference and the W and H
    #get the user's screen width and height
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    #calculate x and y coordinates to paint the app centered on the screen
    x = int((screen_width/2) - (w/2))
    y = int((screen_height/2) - (h/2))
    centerGeo = self.master.geometry ('{}x{}+{}+{}'.format(w,h,x,y))
    return centerGeo

#catch if the user clicks on the upper-right X to ensure they would like to close
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Ok to exit the application?"):
...
def create_db(self):
    ...
    with conn:
        ...
        conn.commit() #must commit the changes and then close the db
    conn.close()
    first_run(self)
...
def onSelect(self,event):
    ...
    with conn:
        ...
        #this will return the tuple and we can slice it in to 4 parts using the data [] during iteration
        for data in varBody:
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0:
                ...
                tkinter.messagebox.showerror("Name Error","'{}' already exists in teh database. Please choose a different name or update the existing name.".format(var_fullname))
                onClear(self)
        conn.commit()
        conn.close()
    else:
...
def onDelete(self):
    ...
    with conn:
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = tkinter.messagebox.askokcancel("Delete Confirmation", "All information associated with, ({}) \nwill be permenantly deleted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
######             onRefresh(self) # update the listbox of the changes
            ...
            confirm = tkinter.messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deleted at this time. \n\nPlease add another record first before you can delete ({}).".format(var_select,var_select))
    conn.close()
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    tkinter.messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
